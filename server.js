...
// ========== Multer for khutbah PDFs ==========
const storage = multer.diskStorage({
  destination: (req, file, cb) => cb(null, KHUTBAHS_DIR),
  filename: (req, file, cb) => {
    const safe =
      Date.now() + "-" + file.originalname.replace(/[^a-zA-Z0-9.\-\u0600-\u06FF ]/g, "_");
    cb(null, safe);
  },
});
const upload = multer({
...
app.use(
  session({ secret: "fahd_classic_secret", resave: false, saveUninitialized: true })
);
...
.title-main{font-size:20px;font-weight:700}.title-sub{font-size:13px;color:#666}
...
<div><div class="title-main">فهد بن عبدالله الجربوع</div><div class="title-sub">منصة علمية للفتاوى والمقالات والخطب</div></div>
</a>
...
<div class="container"><div class="row"><div class="col-lg-8">${bodyHtml}</div><div class="col-lg-4">
...
<div class="card-modern"><h5 class="section-title">عن المنصة</h5>
...
// =====================
// Routes (Home, Fatwas, Articles, Videos, Khutbahs, Ask, Search, Admin)
// جميعها تستخدم load/save مع تخزين محلي دائم
// =====================
...
app.get("/", (req, res) => {
  ...
  <ul class="list-unstyled">${fatwas
    .map((f) => `<li class="mb-1"><a href="/fatwas/${f.id}">${esc(f.title)}</a></li>`)
    .join("")}</ul>
  </div>
  ...
  <ul class="list-unstyled">${articles
    .map((a) => `<li class="mb-1"><a href="/articles/${a.id}">${esc(a.title)}</a></li>`)
    .join("")}</ul>
  </div>
  ...
  <ul class="list-unstyled">${videos
    .map((v) => `<li class="mb-1"><a href="/videos/${v.id}">${esc(v.title)}</a></li>`)
    .join("")}</ul>
  </div>

  <div class="classic-card">
  <h4 class="section-title">الخطب المفرغة (PDF)</h4>
  <ul class="list-unstyled">${khutbahs
    .map((k) => `<li class="mb-1"><a href="/khutbahs/${k.id}">${esc(k.title)}</a></li>`)
    .join("")}</ul>
  </div>
  `;
...
// باقي Routes (fatwas/:id, articles/:id, videos/:id, khutbahs/:id, ask, admin) 
// تمامًا كما هي في كودك السابق، مع التأكد أن كل load/save تعمل على ملفات JSON المحلية.
// رفع PDF يظل في data/uploads/khutbahs.
